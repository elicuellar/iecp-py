...
class TestDebouncerMultipleRapidMessages:
    ...
    async def test_groups_multiple_rapid_messages_into_a_single_batch(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
        ...
        await debouncer.handle_event(msg1)
        timer.advance(500)
        await debouncer.handle_event(msg2)
        timer.advance(500)
        await debouncer.handle_event(msg3)
...
class TestDebouncerTimerReset:
    ...
    async def test_resets_the_timer_on_each_new_message(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(text="first"))
        timer.advance(2500)  # 500ms left on original timer
        await debouncer.handle_event(make_message(text="second"))
...
class TestDebouncerEventOrder:
    ...
    async def test_preserves_event_id_order_in_sealed_batch(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
...
class TestDebouncerIndependentAuthors:
    ...
    async def test_creates_independent_batches_for_different_authors(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
        ...
        await debouncer.handle_event(msg_a)
        timer.advance(500)
        await debouncer.handle_event(msg_b)
...
class TestDebouncerIndependentConversations:
    ...
    async def test_creates_independent_batches_for_different_conversations(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
        ...
        await debouncer.handle_event(msg_conv_a)
        await debouncer.handle_event(msg_conv_b)
...
class TestDebouncerContinuation:
    ...
    async def test_does_not_seal_batch_when_last_message_has_is_continuation(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(text="start"))
        timer.advance(1000)
        await debouncer.handle_event(
    ...
    async def test_seals_after_normal_message_following_a_continuation(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
    ...
    async def test_never_seals_if_all_messages_are_continuations(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(text="a", is_continuation=True))
        timer.advance(3000)  # extends
        await debouncer.handle_event(make_message(text="b", is_continuation=True))
...
class TestDebouncerTypingStart:
    ...
    async def test_resets_the_timer_on_typing_start(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(text="hello"))
        timer.advance(2500)  # 500ms left

        await debouncer.handle_typing_start(CONV_A, AUTHOR_A)
    ...
    async def test_ignores_typing_start_when_no_active_batch(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
...
class TestDebouncerFiltering:
    ...
    async def test_ignores_non_message_event_types(self) -> None:
        ...
        await debouncer.handle_event(event)
        timer.advance(5000)
        assert len(batches) == 0

        await debouncer.destroy()

    async def test_ignores_messages_from_non_human_authors(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
...
class TestDebouncerAdaptiveTiming:
    ...
    async def test_adjusts_timer_based_on_message_cadence_after_sufficient_data(
        self,
    ) -> None:
        timer = FakeTimerProvider()
        ...
        b.clear()
        timer.advance(2000)
        await d.handle_event(make_message(text="4"))
    ...
    async def test_uses_base_ms_with_fewer_than_3_data_points(self) -> None:
        timer = FakeTimerProvider()
        ...
        await debouncer.handle_event(make_message(text="third"))
        timer.advance(3000)
        assert len(batches) == 3

        await debouncer.destroy()

    async def test_computes_median_correctly_with_outliers(self) -> None:
        timer = FakeTimerProvider()
        ...
        b.clear()
        timer.advance(1000)
        await d.handle_event(make_message(text="test"))
        timer.advance(5999)
        assert len(b) == 0
        timer.advance(1)
        assert len(b) == 1
    ...
    async def test_clamps_to_min_ms(self) -> None:
        timer = FakeTimerProvider()
        d = Debouncer(
        ...
        b.clear()
        timer.advance(100)
        await d.handle_event(make_message(text="final"))
        timer.advance(1499)
        assert len(b) == 0
        timer.advance(1)
        assert len(b) == 1
    ...
    async def test_clamps_to_max_ms(self) -> None:
        timer = FakeTimerProvider()
        d = Debouncer(
            DebouncerConfig(base_ms=100, min_ms=1500, max_ms=8000, adaptive=True),
            timer_provider=timer,
    ...
    async def test_always_uses_base_ms_when_adaptive_is_disabled(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(
            DebouncerConfig(
        ...
        for i in range(10):
            await debouncer.handle_event(make_message(text=str(i)))
            timer.advance(2000)

        batches.clear()

        # The 11th message should still use base_ms=2000
        await debouncer.handle_event(make_message(text="final"))
        timer.advance(1999)
        assert len(batches) == 0
        timer.advance(1)
        assert len(batches) == 1
...
class TestDebouncerDestroy:
    ...
    async def test_clears_all_timers_on_destroy(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(author_id=AUTHOR_A, text="a"))
        await debouncer.handle_event(make_message(author_id=AUTHOR_B, text="b"))
    ...
    async def test_ignores_events_after_destroy(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.destroy()
        await debouncer.handle_event(make_message(text="ignored"))

        timer.advance(10000)
        assert len(batches) == 0

    async def test_ignores_typing_start_after_destroy(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        await debouncer.destroy()
...
class TestDebouncerListeners:
    ...
    async def test_supports_removing_listeners_with_off(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches: list[SealedBatch] = []
        ...
        await debouncer.handle_event(make_message(text="msg-1"))
        timer.advance(3000)
        assert len(batches) == 1

        debouncer.off("batch_sealed", listener)
        await debouncer.handle_event(make_message(text="msg-2"))
        timer.advance(3000)
        assert len(batches) == 1  # Listener was removed

        await debouncer.destroy()

    async def test_emits_to_multiple_listeners(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        ...
        await debouncer.handle_event(make_message(text="multi"))
        timer.advance(3000)
...
class TestDebouncerBatchProperties:
    ...
    async def test_generates_a_unique_batch_id_for_each_sealed_batch(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(text="batch-1"))
        timer.advance(3000)

        await debouncer.handle_event(make_message(text="batch-2"))
        timer.advance(3000)
    ...
    async def test_sets_sealed_at_to_current_time_when_sealing(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)

        await debouncer.handle_event(make_message(text="timed"))
        timer.advance(3000)
...
class TestDebouncerInterleaving:
    ...
    async def test_does_not_let_author_a_affect_author_b_timing(self) -> None:
        timer = FakeTimerProvider()
        debouncer = Debouncer(BASE_CONFIG, timer_provider=timer)
        batches = collect_batches(debouncer)
        ...
        await debouncer.handle_event(msg_a2)
